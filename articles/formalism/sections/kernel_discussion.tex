...
\textit{Evaluating the local kernels: }\revisit{Let us consider the case when one of the coordinates coincides with the north pole (more formally this refers to the point $\theta_0 \rightarrow 0$ while moving along the longitude $\phi_0=0$), say $\hat{n}_0=(\theta_0,\phi_0)=(0,0)$. In this case the Euler angles in the $z-y1-z2$ convention are simply given by: $(\alpha,\beta,\gamma) =(\phi_i,\theta_i,0)$, where $(\theta_i,\phi_i)$ denote the coordinates of the location $\hat{n}_i$.} Since the Euler angle $\gamma=0$ when rotations are defined with respect to the pole, the respective kernels simplify to the following forms,
%
...
%
\beq
{}_s a_{\ell m} = D^{\ell}_{m 2}(\phi_j , \theta_j, 0) {{}_s}a_{\ell 2} \,,
\eeq
%
...
We compute the local convolution kernels using the procedure outlined in the discussion above. Since its not easy to imagine how the Euler angles vary as a function of position of the central pixel we depict the kernels at different locations in \fig{fig:vis_kernel}, to give a sense of how these kernels vary across the sphere.
%

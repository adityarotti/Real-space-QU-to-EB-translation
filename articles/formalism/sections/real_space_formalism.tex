...
For this work, we believe it is convenient to work in the z-y1-z2 convention, therefore all future references to Euler angles are to be assumed to be given in this convention. \footnote{The Euler angles in the more standard $z-y-z$ convention are related to those in the $z-y1-z2$ convection by the following rule: $(\alpha,\beta,\gamma)_{z-y-z} =(\gamma,\beta,\alpha)_{z-y1-z2}$ \cite{varshalovich}.}
...
\beq
 ...
 \eeq
 %
 where the terms on either side of the equation are those that appear in \eq{eq:qu2eb_ker_1}. Note that each sub-matrix in \eq{eq:qu2eb_ker_1} is formed by some linear combination of a complex number and its conjugate. The $m$ sum over the product of two spherical harmonic functions with spins $s_1$ and $s_2$ respectively, is given by the following identity \cite{varshalovich},
%

...
\eeq
%
\revisit{where we use the notation $\bar{\eth}^2_{\textrm BL}$ to represent the band limited version of the spin lowering operator, thats relevant in the case of a discretely sampled spin-2 field on the sphere. In the limit of $\ell_{\rm max} \rightarrow \infty$, the $\bar{\eth}^2_{\textrm BL} \rightarrow \bar{\eth}^2$.}
%The operator $\bar{\eth}^2_{\textrm BL}$ is composed of derivative operations and hence has an implicit positional dependence for all numerical purposes which is made explicit in the band limited version of the operator.  

...
def return_euler_angles(theta0,phi0,theta1,phi1):
...
def fn_euler_trig2alpha(nside,cpix,discsize=np.pi,inclusive=False,fact=4):
	...
	cosalpha=np.where(abs(cosbeta)<1.,(np.cos(theta0)*cosbeta - np.cos(theta1))/(np.sin(theta0)*np.sqrt(1.-cosbeta*cosbeta)),0)
...
def fn_euler_trig2gamma(nside,cpix,discsize=np.pi,inclusive=False,fact=4):
	...
	cosgamma=np.where(abs(cosbeta)<1.,(np.cos(theta0) - cosbeta*np.cos(theta1))/(np.sin(theta1)*np.sqrt(1.-cosbeta*cosbeta)),0)

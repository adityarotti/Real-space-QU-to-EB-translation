...
def convert_qu2_equ_bqu_convolve(q,u,theta_cutoff,theta,rad_ker_d,cq=[],cu=[],mask=[]):
	...
	if np.size(cq)!=0 and np.size(cu)!=0:
		for i in pix_list:
			cosbeta,c2amg,s2amg,spix=euler.fn_euler_trig2_alpha_minus_gamma(nside,i,theta_cutoff,inclusive=True,fact=4)
			gbeta=fn_rad_ker_d(cosbeta)
			Dr=c2amg*gbeta ; Di=s2amg*gbeta
